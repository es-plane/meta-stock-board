...
`src/data/catalog.ts` は2026-09-16時点の検証済み相場を含む。単価に「確認済」バッジがあるものは価格ソース（🔗）で裏付け済み、
「参考」は未検証の参考値。GCGはメルカード主役、PTCGはメルカードポケモン部が503のためgamepedia相場＋cardrush＋遊々亭が主役。
実運用では更新エージェントが `priceSources` つきで更新する。
